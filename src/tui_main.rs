use std::{io, sync::{Arc, RwLock}, time::{Duration, Instant}, collections::HashMap};

use crossterm::{terminal::{enable_raw_mode, disable_raw_mode, Clear, ClearType}, event::{self, KeyEvent}, execute, cursor::MoveTo};
...
pub async fn init(tui_accounts: Arc<RwLock<HashMap<String, structs::AccountData>>>, tui_accounts_list: Arc<RwLock<Vec<String>>>) {
        ...
        loop {
            terminal.draw(|rect| {
                ...
                match active_menu_item {
                    structs::MenuItem::Logs => rect.render_widget(render_logs(), chunks[1]), // TODO,
                    structs::MenuItem::Dashboard => {
                        ...
                        let pets_chunks = Layout::default()
                            ...
                            .split(chunks[1]);
                        let (left, right) = render_dashboard(&account_list_state, &account_list, &accounts);
                        rect.render_stateful_widget(left, pets_chunks[0], &mut account_list_state);
            ...
            match rx.recv().await.expect("TODO") {
                structs::Event::Input(event) => match event.code {
                    ...
                    KeyCode::Left | KeyCode::Char('d') => active_menu_item = structs::MenuItem::Dashboard,
                    KeyCode::Right | KeyCode::Char('l') => active_menu_item = structs::MenuItem::Logs,
                    KeyCode::Down => {
                        match active_menu_item {
                            structs::MenuItem::Dashboard => {
    ...
    loop {
        ...
        if event::poll(timeout).expect("poll works") {
            if let CEvent::Key(key) = event::read().expect("can read events") {
                tx.send(structs::Event::Input(key)).await;
            }
        }

        if last_tick.elapsed() >= tick_rate {
            if let Ok(_) = tx.send(structs::Event::Tick).await {
...
pub fn render_logs<'a>() -> Paragraph<'a> {
    let log_items: Vec<_> = LOGGER.logs().iter().rev()
...
fn render_dashboard<'a>(pet_list_state: &ListState, tui_accounts_list: &Vec<String>, tui_accounts: &HashMap<String, structs::AccountData>) -> (List<'a>, Table<'a>) {
    ...
    ]), Row::new(vec![Cell::from(Span::raw(""))]), Row::new(vec![
